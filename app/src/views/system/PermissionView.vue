...
const getMenu = (menuId) => {
  return permissionApi.get(menuId).then(response => {
    if (response.code === 200) {
      return response.data
    }
    return Promise.reject(new Error(response.message || '获取权限信息失败'))
  })
}
...
const submitForm = () => {
  menuRef.value.validate(valid => {
    ...
    if (form.menuId) {
      permissionApi.update(form.menuId, data).then(response => {
        if (response.code === 200) {
      ...
      permissionApi.create(data).then(response => {
        if (response.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    permissionApi.delete(row.menuId).then(response => {
      if (response.code === 200) {
