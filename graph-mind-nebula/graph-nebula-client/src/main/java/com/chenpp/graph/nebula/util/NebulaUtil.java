...
public class NebulaUtil {
    ...
    private String buildCreateTag(GraphEntity entity) {
        ...
        String properties = entity.getProps().stream().map(prop -> prop.getName() + " " + prop.getDataType().name())
                .collect(Collectors.joining(", "));
        tagBuilder.append("CREATE TAG IF NOT EXISTS ").append(entity.getName()).append(" (").append(properties).append(")");
    ...
    private String buildCreateEdge(GraphRelation relation) {
        ...
        String properties = relation.getProps().stream().map(prop -> prop.getName() + " " + prop.getDataType().name())
                .collect(Collectors.joining(", "));
        edgeBuilder.append("CREATE EDGE IF NOT EXISTS ").append(relation.getName()).append("(").append(properties).append(")");
